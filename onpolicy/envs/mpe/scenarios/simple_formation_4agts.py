...
class Scenario(BaseScenario):
    ...
    def make_world(self,args):
        ...
        for i, d_obs in enumerate(world.dynamic_obstacles):
            d_obs.id = i
            d_obs.color = np.array([0.55, 0.15, 0.25])
            d_obs.size = 0.12
            d_obs.max_speed = 0.3
            d_obs.max_accel = 0.5
        ...
        for i, obs in enumerate(world.obstacles):
            obs.id = i
            obs.color = np.array([0.25, 0.25, 0.25])
    ...
    def reset_world(self, world):
        ...
        # init_pos_ego = np.array([[-0.3, 0.], [-1., 0.2], [0.3, 0.1], [1., -0.2]])
        init_pos_ego = np.array([[0., 0.], [-1., 0.], [0., 1.], [1., 0.]])
        init_pos_ego = init_pos_ego + np.random.randn(*init_pos_ego.shape)*0.05
        H = np.array([[0., 0.], [-1., 0.], [0., 1.], [1., 0.]])
        for i, ego in enumerate(world.egos):
            if i==0:
                ego.is_leader = True
                ego.goal = np.array([0., 6.])
            ego.done = False
        ...
        init_pos_obs = np.array([[-0.5, 2.4], [0.6, 3.], [0.8, 2.5], [1.3, 3.]])
        sizes_obs = np.array([0.2, 0.2, 0.2, 0.2])
        for i, obs in enumerate(world.obstacles):
            obs.done = False
        ...
        init_pos_d_obs = np.array([[-4., 4.], [4., 4.], [-4., 7.], [4., 7.]])
        init_direction = np.array([[1., -1.], [-1., -1.], [1., -1.], [-1., -1.]])
        for i, d_obs in enumerate(world.dynamic_obstacles):
            d_obs.done = False
            d_obs.t = 0
