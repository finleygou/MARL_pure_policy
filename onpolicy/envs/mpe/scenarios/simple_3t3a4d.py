...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        self.assign_list = [1, 0, 2]

        # properties and initial states for agents
        for i, target in enumerate(world.targets):
            if i == 0: 
                target.done = False
                target.state.p_pos = np.array([0.0, 5.0])
                ...
                target.done = False
                target.state.p_pos = np.array([-1.0, 1])
                target.state.p_vel = np.array([target.max_speed, 0.0])
                target.state.V = np.linalg.norm(target.state.p_vel)
                target.state.phi = 0.
                ...
                target.done = False
                target.state.p_pos = np.array([1.2, -4])
                target.state.p_vel = np.array([target.max_speed, 0.0])
                target.state.V = np.linalg.norm(target.state.p_vel)
                target.state.phi = 0.
                target.attacker = [j for j in range(self.num_attacker) if self.assign_list[j]==i]
        ...
        for i, attacker in enumerate(world.attackers):
            if i == 0: 
                attacker.done = False
                attacker.state.p_pos = np.array([20.0, 5])
            ...
            if self.init_assign:
                for i, defender in enumerate(defender_list):
                    for j, attacker in enumerate(attacker_list):
                        T[i, j] = get_init_cost(attacker, defender, world.targets[attacker.true_target])
                self.init_assign = False      
            else:
                for i, defender in enumerate(defender_list):
                    for j, attacker in enumerate(attacker_list):
                        T[i, j] = get_cost(attacker, defender, world.targets[attacker.fake_target])
            # print('T is:', T)
